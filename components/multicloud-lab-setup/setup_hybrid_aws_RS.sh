...
####cleansups
ctx logger info "delete old openstack-network dep"
cfy deployments delete "openstack-example-network"  >> /tmp/lab_status.txt 2>&1
cfy blueprints delete "openstack-example-network" >> /tmp/lab_status.txt 2>&1
...
ctx logger info "Create new openstack-network dep"
cfy deployments create -b "openstack-network-bp"  openstack-network -i external_network_id=dda079ce-12cf-4309-879a-8e67aec94de4 >> /tmp/lab_status.txt 2>&1
cfy executions start install -d "openstack-network" >> /tmp/lab_status.txt 2>&1 &

ctx logger info "Create AWS-network dep"
cfy deployments create -b "aws-network-bp"  aws-network >> /tmp/lab_status.txt 2>&1
ctx logger info "Install AWS-network dep"
cfy executions start install -d "aws-network" >> /tmp/lab_status.txt 2>&1

#### create and isnatll deployments of infra VMs + kube
ctx logger info "Create Kubernetes dep"
cfy deployments create -b "kubernetes-bp"  kubernetes -i public_subnet_cidr=10.1.18.0/24 -i external_network_id=dda079ce-12cf-4309-879a-8e67aec94de4 -i region_name=RegionOne -i image_id=0847a4c0-99bf-4cb7-9cec-b019eb2fae99 -i flavor_id=3 >> /tmp/lab_status.txt 2>&1
ctx logger info "Install Kubernetes dep"
...
ctx logger info "Create AWS db & lb dep"
