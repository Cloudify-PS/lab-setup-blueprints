...
#cfy plugins upload https://github.com/cloudify-cosmo/cloudify-gcp-plugin/releases/download/1.4.4/cloudify_gcp_plugin-1.4.4-py27-none-linux_x86_64-centos-Core.wgn -y https://github.com/cloudify-cosmo/cloudify-gcp-plugin/releases/download/1.4.4/plugin.yaml  >> /tmp/cfy_status.txt 2>&1 &
#cfy plugins upload https://github.com/cloudify-incubator/cloudify-azure-plugin/releases/download/2.1.2/cloudify_azure_plugin-2.1.2-py27-none-linux_x86_64-centos-Core.wgn -y https://github.com/cloudify-incubator/cloudify-azure-plugin/releases/download/2.1.2/plugin.yaml  >> /tmp/cfy_status.txt 2>&1 &
...
### DB LB APP Wordpress
cfy blueprints upload https://github.com/cloudify-community/blueprint-examples/releases/download/4.5.5-7/db-lb-app-app.zip -n application.yaml -b "kube-wordpress-app"  >> /tmp/cfy_status.txt 2>&1 &
...
cfy deployments create -b "kubernetes"  kubernetes >> /tmp/cfy_status.txt 2>&1
cfy executions start install -d "kubernetes" >> /tmp/cfy_status.txt 2>&1 &

######create and install deployments of apps on infra VMs +kube app

cfy deployments create -b "aws-db-app" aws-db -i infrastructure--resource_name_prefix=db -i network_deployment_name=openstack-network >> /tmp/cfy_status.txt 2>&1 &
cfy deployments create -b "aws-lb-app" aws-lb  -i infrastructure--resource_name_prefix=lb -i network_deployment_name=openstack-network -i database_deployment=aws-db >> /tmp/cfy_status.txt 2>&1

cfy executions start install -d "aws-db" >> /tmp/cfy_status.txt 2>&1
cfy executions start install -d "aws-lb" >> /tmp/cfy_status.txt 2>&1

cfy deployments create -b "kube-wordpress-app" wordpress -i network_deployment_name=openstack-network -i load_balancer_deployment=aws-lb -i kubernetes_deployment=kubernetes >> /tmp/cfy_status.txt 2>&1 
